...
| error handling | Error handling behavior for failures is documented. | |
...
| graceful failure behavior | Invalid input paths return clean structured errors. | |
| chaos restart proof | Evidence shows service restart behavior after forced failure. | |
| failure modes documented | Failure modes and recovery expectations are documented. | |
